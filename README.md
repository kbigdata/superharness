...
| `skills proposed` / `skills promote <name>` | 학습으로 제안(격리)된 스킬 목록 / 활성화 승격 |
...
src/superharness/
  ...
  skills/         skill · registry · keyword_detector · injector · writer(학습 게이트) · builtin/*.md
...
- ✅ **자동 스킬 생성(`learner`)** — 검증 통과 세션에서 재사용 스킬 추출(격리/제안 → 승격). `team --learn` 참고
- `wiki`류 스킬(기존 `notepads/`·`project-memory.json` 활용), 추출 스킬 refine/버전관리
