...
### A. 라이브러리로 설치 ([최신 릴리스](https://github.com/kbigdata/superharness/releases/latest) · `v0.2.0`)

다른 프로젝트에서 엔진으로 가져다 쓸 때. 세 가지 방법 중 택1:

```bash
# 1) git 태그 의존 (권장) — uv가 클론→빌드→설치
uv add "superharness @ git+https://github.com/kbigdata/superharness@v0.2.0"

# 2) 릴리스 첨부 wheel
uv pip install \
  https://github.com/kbigdata/superharness/releases/download/v0.2.0/superharness-0.2.0-py3-none-any.whl

# 3) 소스 배포본(sdist)
uv pip install \
  https://github.com/kbigdata/superharness/releases/download/v0.2.0/superharness-0.2.0.tar.gz
```
...
```
실 Claude 백엔드까지: `uv add "superharness[anthropic] @ git+https://github.com/kbigdata/superharness@v0.2.0"`.
...
**상태**: v0.2.0 — 4대 핵심 패턴 + 자동 스킬 생성(learner)·refine/버전관리·의미 dedup·CI. mock·실 Claude 양쪽 동작.
