...
```bash
uv run pytest -q       # 55개 테스트 — 오프라인(mock), 네트워크/키 불필요
uv run ruff check .    # 린트
uv run mypy src        # 타입 체크
```

- **55개 테스트 · 라인 커버리지 95%** (오프라인 53 + 라이브 2 opt-in).
...
src/superharness/
  ...
  skills/         skill · registry · keyword_detector · injector · writer(학습 게이트) · versions(버전/롤백) · builtin/*.md
...
- `wiki`류 스킬(기존 `notepads/`·`project-memory.json` 활용), 원격 상태, CI 확장
