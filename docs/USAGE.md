...
- 패키지/환경 관리: [**uv**](https://docs.astral.sh/uv/) (이 머신의 기본 `python3`가 3.9라 uv로 3.12를 조달)
...
> 파이썬 버전은 레포의 `.python-version`(3.12)으로 핀되어 있다.
...
설치하면 venv에 `superharness` 명령과 `python -m superharness` 진입점이 생긴다.
이후 명령은 `uv run <cmd>`로 실행하거나 `source .venv/bin/activate` 후 실행한다.

> 참고: uv 없이 진행하려면, 3.11+ 인터프리터를 직접 마련한 뒤(`pyenv` 등)
> `python -m venv .venv && .venv/bin/pip install -e ".[dev]"`로도 동일하게 동작한다.
...
API 키 없이(기본 `mock` 프로바이더) 전부 동작한다. 아래 예시는 venv가 활성화된 상태를 가정한다
(`source .venv/bin/activate`). 활성화하지 않았다면 각 명령 앞에 `uv run`을 붙인다.
...
SUPERHARNESS_PROVIDER=anthropic uv run superharness ask "한 줄 요약" --tier low
```
...
```bash
uv run pytest -q                          # 전체(오프라인, 네트워크/키 불필요)
uv run pytest tests/test_e2e_demo.py -q   # 단일 파일
...
venv가 활성화되지 않았다. `source .venv/bin/activate` 후 실행하거나, `uv run superharness ...`
또는 `.venv/bin/superharness ...`로 실행.

**Q. `Package 'superharness' requires a different Python: 3.9...`**
기본 python이 3.9다. uv로 3.12 venv를 만든다: `uv venv --python 3.12 && uv pip install -e ".[dev]"`.

**Q. `ProviderNotInstalled: anthropic 프로바이더는 ...`**
`anthropic` extra 미설치. `uv pip install -e ".[dev,anthropic]"` 후 `ANTHROPIC_API_KEY` 설정.
...
*이 문서는 슈퍼하네스 v0.3.0 구현 기준이다. 코드가 바뀌면 동작/이름이 달라질 수 있으니, CLI는 `--help`로, 동작은 `tests/`로 최종 확인하라.*
